...
export default {
  name: "Dashboard",
  methods: {
    ...mapActions('dashboardData', ['fetchFactoryDashboardData']),
    handleViewClick(status) {
      return this.$router.push({ name: "FactoryOrders", params: {status: status}});
    },
  },
  computed: {
    ...mapState('dashboardData', ['factoryDashboardData']),
  },
  created() {
    this.userData = JSON.parse(localStorage.getItem("user"));
  },
  async mounted() {
    this.dashboardData = await this.fetchFactoryDashboardData();
  }
}
</script>
