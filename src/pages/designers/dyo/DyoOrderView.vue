<template>
  <div id="product">
    ...
    <div class="flex flex-wrap">
      ...
      <div class="w-full">
        <p class="text-xl font-semibold mb-4 float-left">Order ID - #{{ order.id }} ({{order.type}})</p>
        <p class="text-xl font-semibold mb-4 float-right">
          ...
          <el-button type="success" size="mini" v-if="order.status === 3">Sent To Factory</el-button>
          ...
          <el-button type="warning" size="mini" v-if="order.payment_status === 1">PENDING</el-button>
          <el-button type="success" size="mini" v-if="order.payment_status === 2">PAID</el-button>
          <el-button type="danger" size="mini" v-if="order.payment_status === 3">FAILED</el-button>
            </span>
      ...
      <div class="w-full bg-white border text-blue-400 rounded-lg flex-row items-center p-6 mb-6 xl:mb-0">
        ...
        <div class="w-full">
            ...
            <table>
              ...
              <tr v-for="(item, index) in order.items" :key="item.id">
                ...
                <td>
                  ...
                  <router-link v-if="order.type == 'DYO'"  target="_blank" class="el-icon-link" :to="{ name: 'DyoOrderItemView', params: { 'id': order.id, 'itemId': item.id} }">
                    view Item
                  </router-link>
                </td>
                <td>
          ...
          <div>
            ...
            <div class="mt-4" v-if="order.status==1 || order.status==2">
              <div class="mb-4">
                <el-alert
                    title="Send to factory"
                    type="warning"
                    description="Please check all the designs before sending this order to the factory.
                  If everything is fine, click the bellow button to send this order to the factory.">
                </el-alert>
              </div>

              <el-button type="primary" v-on:click="updateStatusHandler">Send To Factory</el-button>
            </div>
          </div>
